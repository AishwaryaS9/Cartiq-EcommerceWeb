...
const Footer = () => {
    ...
    const XIcon = () => (
        <svg
            ...
            className="transition group-hover:scale-110"
        >
            <path d="M3 2L8.3 9.1L3 18H6.3L9.9 12.3L13.8 18H17L11.5 10.5L16.5 2H13.2L9.9 7.3L6.7 2H3Z" />
        </svg>
    );
    ...
    const socialIcons = [
        { label: "Facebook", link: "https://facebook.com", icon: FacebookIcon },
        { label: "Instagram", link: "https://instagram.com", icon: InstagramIcon },
        { label: "X (formerly Twitter)", link: "https://x.com", icon: XIcon },
        { label: "LinkedIn", link: "https://linkedin.com", icon: LinkedinIcon },
    ];
    return (
        <footer
            className="bg-secondary text-slate-500"
            role="contentinfo"
        ...
        >
            <div className="max-w-7xl mx-auto px-6 sm:px-10">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30">
                    ...
                    <div className="flex-1 min-w-[250px]">
                        <Link
                            ...
                            className="text-4xl font-semibold text-slate-700 hover:opacity-90 transition"
                        >
                            <span className="text-primary">Cartiq</span>
                            <span className="text-customBlack text-5xl leading-0">.</span>
                        </Link>
                        ...
                        <div className="flex gap-4 mt-6" aria-label="Social media links">
                            {socialIcons.map(({ label, link, icon: Icon }) => (
                                <Link
                                    key={label}
                                    href={link}
                    ...
                    >
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-medium text-slate-700 md:mb-5 mb-3 uppercase tracking-wide">
                                    {section.title}
                                </h3>
                ...
                {/* Footer Bottom */}
                <p className="py-4 text-sm text-center md:text-left text-slate-500">
                    &copy; {new Date().getFullYear()} <strong>Cartiq</strong>. All rights reserved.
                </p>
            </div>
        </footer>
    );
